...
.controller('FileCtrl',['$scope','$log','FileService','HomeService','$routeParams','Alertify','DB','$http',function($scope,$log,FileService,HomeService,$routeParams,Alertify,DB,$http){
...
.controller('CommitCtrl',['$scope','$log','$routeParams','Alertify','FileService','CommitService','DB','$http',function($scope,$log,$routeParams,Alertify,FileService,CommitService,DB,$http){
    ...
    $scope.pushChanges = function(){
        DB.getDB().then((data) => {
            return $http.post('http://localhost:3000/commit/pushToDB',data);
        })
        .then((data) => {
            if(data.data.success)
                Alertify.success('Changes Pushed To Server');
            else{
				$log.log("err = ",data.data);
				Alertify.error('Some error occurred');
			}
        })
    }
